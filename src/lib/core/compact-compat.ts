...
import { cloneLayoutItem, bottom } from "./layout";
import { getFirstCollision, collides } from "./collision";
...
function compactItemInternal(
  ...
  while ((collision = getFirstCollision(compareWith, l)) !== undefined && !allowOverlap) {
    if (compactH) {
...
export function compact(
  ...
  const out: LayoutItem[] = Array.from({
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    if (!l.static) {
      l = compactItemInternal(compareWith, l, compactType, cols, sorted, allowOverlap, b);
