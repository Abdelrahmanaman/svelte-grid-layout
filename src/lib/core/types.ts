...
export interface PositionStrategy {
   ...
   */
  calcStyle(pos: Position): Record<string, string | number>;
...
export interface DragConfig {
  ...
  /**
   * Minimum actual pixel overlap ratio before drag collisions reflow other items.
   * Set to 0 for immediate collision resolution.
   * @default 0.25
   */
 ...
 * Generic type B allows custom breakpoint strings.
 */
export type BreakpointCols<B extends Breakpoint = Breakpoint> = Record<B, number>;

/**
 * Map of breakpoint name to layout.
 * Generic type B allows custom breakpoint strings.
 */
export type ResponsiveLayouts<B extends Breakpoint = Breakpoint> = Partial<Record<B, Layout>>;
